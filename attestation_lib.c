...
inline uint32_t next_addr() {
  ...
  if(addr < 16384)  next_addr();
  if(addr > 65504)  next_addr();
...
//  printf("ptr: %u, *ptr: %u, **ptr: %u, addr: %u\n", ptr, *ptr, **ptr, addr);
//  printf("Returning %p: %u\n", ptr, *ptr);
 // return *a;
    return 3; 
}
...
uint64_t attestation(uint32_t nonce) {
  ...
  //for(i = 0, byte = 0; i < MAX_MEMORY_ACCESSES; i++) {
    ...
    tmp %= CHECKSUM_LENGTH;
    tmp = checksum[tmp];
    tmp = read_byte_far(current_addr) ^ tmp;
    tmp += last_addr;
